...
Compared:
Winning Ticket initialization
Random Reinitialization baseline
CONV-2 on CIFAR-10
(Primary area of contribution)
Two-layer convolutional neural network
Architecture:
Conv(64) → Conv(64) → Pool
FC(256) → FC(256) → Output(10)
...
My Contributions
My primary focus was the CONV-2 structured pruning experiments on CIFAR-10. Contributions included:
Implemented and tested iterative structural pruning procedures for convolutional neural networks
...
Helped compare structured pruning results against findings from the original Lottery Ticket Hypothesis paper
Collaborated with teammates on experiment design, debugging, and result interpretation
Key Findings
Structured pruning was able to significantly reduce model size while preserving much of the original test accuracy
...
Training difficulty increased substantially as networks became more compressed
Results suggest that identifying structured lottery tickets is more difficult than identifying unstructured sparse subnetworks
Technologies Used
Python
PyTorch
NumPy
